...
MAX_LENGTH = 512
LANG_CODES = {'en': 'en_XX', 'cn': 'zh_CN'}

def get_model_and_auxiliaries(args):

    tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    if 'mbart' in args.model_name:
        tokenizer.src_lang = LANG_CODES[args.src_lang]
        tokenizer.tgt_lang = LANG_CODES[args.tgt_lang]
...
def store_results(inputs, predictions, labels, metrics, output_dir, prefix):
    ...
    with open(os.path.join(output_dir, prefix + 'preds.jsonl'), 'w') as preds_out:
        print('storing results under', os.path.join(output_dir, prefix + 'preds.jsonl'))
...
def main(args):
    ...
    if args.do_train:
        ...
        store_results(inputs, predictions, labels, metrics, output_dir, '{}test_'.format(args.test_prefix))
    elif args.do_predict_zero_shot:
        predictions, _, _  = trainer.predict(test_dataset)
        inputs, predictions, labels = postprocess_test_data(test_dataset.inputs, predictions, test_dataset.targets, tokenizer)
        metrics = compute_test_metrics(predictions, labels)
        store_results(inputs, predictions, labels, metrics, output_dir, 'zero_shot_{}test_'.format(args.test_prefix))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Model Training')
    parser.add_argument("--data_path", type=str, help="Path to data")
    parser.add_argument("--test_prefix", type=str, default='', help="empty string or 'gold_'")
